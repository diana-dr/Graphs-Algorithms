...
class Graph:
    ...
    def findShortestPath(self, s, d):
        """
        :param firstEnd: the starting vertex
        :param secondEnd: the ending vertex
        :return: the shortest path between the given vertices
        """
        ...
        for i in self._outbound:
            if not visited[i]:
                queue.append(i)
                visited[i] = True  # If BFS is complete without visited d
        return False
    ...
    def bfs(self, startVertex):
        ...
        while queue != []:
            ...
            children = self.parseOutboundNeighbours(currVertex)
